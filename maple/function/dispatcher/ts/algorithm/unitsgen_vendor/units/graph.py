...
try:
    ...
    def symbol_pos_to_xyz_file(symbols, coords, path, title=''):
        import numpy as _np
        c = _np.asarray(coords)
        with open(path, 'w') as _f:
            _f.write(f'{len(symbols)}\n{title}\n')
            for _s, _xyz in zip(symbols, c):
                _f.write('%-2s %18.10f %18.10f %18.10f\n' % (_s, _xyz[0], _xyz[1], _xyz[2]))
    def MolFormatConversion(*a, **k):
        raise RuntimeError('MolFormatConversion needs openbabel (not on native path)')
