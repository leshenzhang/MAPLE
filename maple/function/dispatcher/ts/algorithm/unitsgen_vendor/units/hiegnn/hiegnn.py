# [native-surgery]
"""
qq: revised from equiformer_v2_oc20.py
note that this version donot support pbc
"""
...
from .dependencies.scn.smearing import GaussianSmearing, LinearSigmoidSmearing, SigmoidSmearing, SiLUSmearing
...
from units._native_scatter import radius_graph
...
from .dependencies.so2_ops import SO2_Convolution
...
class HiEGNN(nn.Module):
    """
    HiEGNN is derived from Equiformer with graph attention built upon SO(2) convolution and feedforward network built upon S2 activation
    ...
    def __init__(
        ...
        max_neighbors=500,
        max_radius=5.0,
        max_num_elements=60,  # qq: to some large number
        num_layers=5,
        sphere_channels=64,
        ...
        self.add_node_feat = add_node_feat
        self.feat_dim = feat_dim
        self.num_resolutions = len(self.lmax_list)
        self.sphere_channels_all = self.num_resolutions * self.sphere_channels
        self.feat_bridge = nn.Linear(self.feat_dim, self.sphere_channels)
    ...
    def forward(self, z, pos, feat, batch, edge_index=None):
        ...
        # print(x.embedding.shape,edge_degree.embedding.shape)
        if self.add_node_feat:
            feat = self.feat_bridge(feat)
            x.embedding[:,0,:] = x.embedding[:,0,:] + feat
        
            #x.embedding = torch.cat([x.embedding,feat],dim=1)
