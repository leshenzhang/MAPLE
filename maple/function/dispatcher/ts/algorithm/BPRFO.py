...
class BatchPRFO:
    ...
    def __init__(self,
                 ...
                 hessian_recalc_quality: bool = False,  # [10] recalc on measured staleness (OFF: D-261 null)
    ...
    def run(self, mols,
        ...
        while outer_it < self.max_outer_iter and len(atoms_list) > 0:
            ...
            if self.hessian_mode == "iterative":
                ...
                # cadence_due is consumed by the [9] partial-refill guard below, so it
                # must stay defined on every path.
                if self.hessian_recalc_quality:
                    cadence_due = ((outer_it == 1) or qual_fire)
            ...
            if E_old_is_set:
                ...
                # Under a [9] partial refill only the _kpart newcomers get FD columns,
                # so the column term must be restricted to them; the base forward is
                # still evaluated for the whole batch.
                _n_at = (self._L_vec // 3)
                _n_col = _n_at[-_kpart:] if _kpart > 0 else _n_at
                _fdm = str(getattr(calc, "_fd_mode", "central")).lower()
                _hmode = str(getattr(calc, "hessian_mode", "numerical")).lower()
                _per_col = 3 if (_hmode == "autograd" or _fdm == "forward") else 6
                self._ge_hess += int(_n_col.sum().item()) * _per_col + int(_n_at.numel())
                if _kpart > 0:
            ...
            # Bofill/PSB/SR1 update: only if we did not just recalculate and at least
            # one step was accepted. [8] the batched update functions no-op rows outside
            # their mask, so in lean mode we skip the host-side .any() sync entirely.
