...
def run_smd(npulls=8, steps=200000, lam1=5.5, seed=20240601,
    ...
    # Work-vs-lambda monotonicity / correlation are STATISTICAL trend gates that
    # sharpen with trajectory length. Apply the STRICT production thresholds only
    # to production-length pulls (>=50k steps; the B-70 200k production gave
    # mono=0.789); a short CI smoke (~8k steps) runs near mono~0.5 / corr~0.7 from
    # finite-sample noise, so use relaxed thresholds there -- still catch a broken
    # pull (no trend at all) but tolerate smoke-length noise. Production unchanged.
    mono_thr, corr_thr = (0.7, 0.8) if steps >= 50000 else (0.4, 0.5)
    assert mono_min > mono_thr, (
        f"WORK-vs-LAMBDA not monotonic-ish (binned trend {mono_min:.3f} <= {mono_thr} "
        f"at steps={steps})")
    assert corr_min > corr_thr, (
        f"WORK not positively correlated with lambda ({corr_min:.3f} <= {corr_thr} "
        f"at steps={steps})")
