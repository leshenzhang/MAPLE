...
# EV2HARTREE re-exported from the base (the single unit source). Kept importable at
# module scope (``from ..._maceoff_batch_calculator import EV2HARTREE``) for the tests.
...
@register_batch_calculator
class MaceOffBatchCalc(BatchCalcABC):
    """Batched MACE-OFF calculator (mace-package native multi-graph batching).

    Retrofit (BatchCalcABC): registers via ``@register_batch_calculator`` and declares
    the capability protocol so the batch factory + dispatcher predicates recognize it
    DECLARATIVELY (no hard-coded name set). It KEEPS its own validated multi-graph
    forward + prepare + coord-ops + the batched-MD surface (stress / NPT cells / REST2
    tempering) that the MD core drives -- those override the base's generic equivalents.
    ``_forward``/``_build_topology`` are the base-contract hooks over the SAME validated
    math; the edge builder (``_build_edges_gpu`` + ``_build_edges_pbc``) carries the
    periodic branch. This is an MD forces/stress backend, NOT a saddle/Hessian backend:
    ``get_efh_gpu`` is intentionally not provided (raises) -- use MACEBatchCalc / the
    MACE-autograd batch calc for a batched Hessian.
    """

    # ---- capability protocol (BatchCalcABC declarative attrs) --------------
    MODEL_NAMES = ("maceoff", "maceoff23_medium")
    MODEL_ENERGY_UNIT = "eV"                 # mace returns eV / eV.A^-1
    MODEL_DTYPE = torch.float64              # MACE-OFF wrapper is f64 (fp32 opt-in via set_precision)
    SUPPORTED_HESSIAN_MODES = ()             # MD forces/stress backend; no batched Hessian
    HAS_HVP = False
    SUPPORTS_COUPLING = False                # pure local MLIP; block-diagonal batch is isolated
    BATCHABLE = True                         # true multi-graph batching
    ...
    def __init__(self,
        ...
        # base.__init__ resolves device/dtype + inits the generic prepared-state for the
        # BatchCalcABC contract/registry. This calc keeps its OWN prepare()/coord-ops/
        # get_ef_gpu (validated) which override the base, so the generic layout stays
        # unused; the explicit device line below preserves the pre-retrofit resolution.
        ...
        # make_batch_calc forwards model_path=None when the caller gives none; restore
        # the shipped default so the registry/factory path loads the bundled checkpoint.
    ...
    def prepare(self, atoms_list, fixed_nmax: int = None):
        ...
        self._build_topology(atoms_list)

    def _build_topology(self, atoms_list):
        """BatchCalcABC topology hook. Delegates to the validated static-cache builder
        (geometry-invariant batch dict + GPU candidate pairs + PBC per-replica cell).
        Invoked from this class's own ``prepare`` (which keeps the validated layout)."""
    ...
    def _forward(self, coord: torch.Tensor, need_graph: bool = False):
        """BatchCalcABC forward-contract hook over the validated MACE-OFF multi-graph
        forward -> (E_eV (B,), F_all_eV (N,3), leaf|None) in NATIVE eV (a base
        ``get_ef_gpu`` would convert once via MODEL_ENERGY_UNIT). This class's OWN
        ``get_ef_gpu`` (which returns Hartree directly + scatters into the padded
        buffer) is authoritative for the MD path; this hook exists so the class
        conforms to the base's abstract ``_forward`` contract and reuses the SAME
        edge build + model call. ``need_graph=True`` returns the grad-carrying force
        field + the requires_grad position leaf; TF32 is scoped exactly as in
        ``_batched_ef_eV`` (byte-identical to the validated forward)."""
        if self.B == 0:
            z = torch.zeros((0,), dtype=self.dtype, device=self.device)
            return z, torch.zeros((0, 3), dtype=self.dtype, device=self.device), None
        leaf = coord.detach().to(self.device, self.dtype).requires_grad_(True)
        edge_index, shifts, unit_shifts = self._build_edges_gpu(leaf)
        d = dict(self._static)                       # shallow copy; static tensors reused
        d["positions"] = leaf
        d["edge_index"] = edge_index
        d["shifts"] = shifts
        ...
        try:
            if _want_tf32:
                ...
                torch.backends.cudnn.allow_tf32 = True
            out = self.model(d, compute_force=True, training=False)
            E_eV = out["energy"].reshape(-1).to(self.dtype)      # (B,)
            F_all_eV = out["forces"].to(self.dtype)              # (N,3)
        finally:
            torch.backends.cuda.matmul.allow_tf32 = _prev_tf32
            torch.backends.cudnn.allow_tf32 = _prev_cudnn_tf32
        if need_graph:
            return E_eV, F_all_eV, leaf
        return E_eV.detach(), F_all_eV.detach(), None

    def get_efh_gpu(self, movable_masks=None, mode=None, **kw):
        """Not provided: MaceOffBatchCalc is the batched MD FORCES/STRESS backend
        (get_ef_gpu / get_efs_gpu / get_stress_gpu), not a saddle/Hessian backend, and
        keeps its own coord layout (no base ``_cols``). Use MACEBatchCalc or the
        MACE-autograd batch calc for the batched per-structure Hessian path."""
        raise NotImplementedError(
            "MaceOffBatchCalc provides get_ef_gpu / get_efs_gpu (MD forces + stress) "
            "but no batched Hessian (get_efh_gpu). Use MACEBatchCalc / the MACE-autograd "
            "batch calc for a batched Hessian.")
