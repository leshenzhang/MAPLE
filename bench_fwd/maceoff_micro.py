...
def main():
    ...
    json.dump(RES, open(OUT, "w"), indent=1)
    n_ok = sum(1 for v in RES["arms"].values() if "error" not in v)
    if n_ok == 0:
        print("ERROR: every arm failed to construct -- nothing was measured. "
              "Failing loudly (a green exit here would be a silent no-op).", flush=True)
        print("MACEOFF_MICRO_FAILED", flush=True)
        raise SystemExit(3)
    print(f"MACEOFF_MICRO_OK ({n_ok}/{len(RES['arms'])} arms measured)", flush=True)
